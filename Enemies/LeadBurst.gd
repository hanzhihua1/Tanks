...
func get_input():
	if get_parent().has_node("Player"):
		...
		var all_bullets = get_tree().get_nodes_in_group(str(id))
			
		aim()
		if len(all_bullets) < 5 and time < OS.get_ticks_msec():
			shoot()
...
func shoot():
	if ($FollowPlayer.get_collider() == Player):
		...
		b.add_to_group(str(id))
		b.start($Sprite2.global_position + Vector2(40, 0).rotated($Lead.rotation + PI/2), $Lead.rotation + PI/2 + rand_range(-0.1, 0.1))
